...
import { createLinkSchema, cashOutSchema, submitPaymentSchema } from "@checkout/core";
...
export function linkRoutes(c: Container): Hono {
  ...
  // Submit a signed XDR from a wallet (desktop/client submission).
  app.post("/:id/submit", async (ctx) => {
     try {
       ...
       if (err instanceof HttpError) return ctx.json({ error: err.message }, err.status);
       throw err;
     }
   });

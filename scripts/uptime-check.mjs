...
// Dedicated least-privilege API key for the synthetic write-path check.
// Requires at minimum `links:read` + `links:write` scopes — stored as a
// repo secret so it never appears in source.  Falls back to unauthenticated
// (which will 401) when the secret is missing, so the check surfaces a
// clear "missing secret" error rather than a cryptic TypeError.
const UPTIME_API_KEY = process.env.UPTIME_API_KEY ?? null;

const TARGETS = [
  { id: "api", label: "API", check: () => checkGet(`${API_URL}/health`) },
  { id: "web", label: "Web dashboard", check: () => checkGet(WEB_URL) },
  { id: "synthetic", label: "Create-link (synthetic)", check: () => checkSyntheticLink(API_URL, UPTIME_API_KEY) },
];
 ...
 *
 * 1. POST /links with bearer auth (apiKey) — proves the public write path
 *    works with the current auth gate.
 * 2. Assert 201.
 * 3. Cancel (soft-delete) the created link immediately via POST /links/:id/cancel
 *    so synthetic rows don't accumulate in the database.
 */
async function checkSyntheticLink(apiUrl, apiKey) {
  const headers = { "content-type": "application/json" };
  if (apiKey) {
    headers.authorization = `Bearer ${apiKey}`;
  }

  const createRes = await fetchWithTimeout(`${apiUrl}/links`, {
    method: "POST",
    headers,
  ...
  // --- Clean up: cancel the throwaway link so rows don't accumulate. --------
  try {
    const body = await createRes.json();
    const linkId = body?.link?.id;
    if (linkId) {
      const cancelRes = await fetchWithTimeout(`${apiUrl}/links/${linkId}/cancel`, {
        method: "POST",
        headers,
      });
      if (!cancelRes.ok) {
        // Non-fatal: the synthetic *write* succeeded, which is the check we
        // care about.  Log but don't fail the uptime probe over cleanup.
        console.warn(`[uptime] warning: failed to cancel synthetic link ${linkId} (HTTP ${cancelRes.status})`);
      }
    }
  } catch {
